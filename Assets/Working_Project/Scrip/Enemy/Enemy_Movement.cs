...
public class Enemy_Movement : MonoBehaviour
{
    ...
    public Transform followTarget = null;/// <Old> Transform
    ...
    private Vector3 scaleEnemyChange;
    /// <summary>
    /// Description:
    ...
    private void Start()
    {
        scaleEnemyChange = transform.localScale;

        if (movementMode == MovementModes.FollowTarget && followTarget == null)
        {
            if (GameManager.instance != null && GameManager.instance.player != null)
            {
                followTarget = GameManager.instance.player.transform;/////<<<<Old--GameManager.instance.player.transform
            }
        }
    }
    ...
    private void MoveEnemy()
    {
        ...
        transform.localScale = scaleEnemyChange;
    ...
    protected virtual Quaternion GetDesiredRotation()
    {
        ...
        {
            case MovementModes.FollowTarget:
                rotation = GetFollowPlayerRotation();////<<<<<<<<Old = GetFollowPlayerRotation
                break;
            case MovementModes.Scroll:
    ...
    private Quaternion GetFollowPlayerRotation()
    {
        ...
        Quaternion rotationToTarget = Quaternion.Euler(0, 0, angle); ///<<<<<<<<<<<<<<<<<<<***EDITE angle-to-target
    ...
    private Vector3 GetScrollDirection()
    {
        ...
        if (camera != null)
        {
            ...
            screenRect.x += 100f;
            screenRect.width -= 200f;
            if (enemyPosition.x > 2.5 || enemyPosition.x < -2.0)
            {
                scaleEnemyChange.x = scaleEnemyChange.x * -1;
